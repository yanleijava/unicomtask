 ...
 * [观测面] 巩固消息头部的可观测元数据注入
 */
public final class SieveFrameDrainDrainAdapter {

    private static final Logger LOG = LoggerFactory.getLogger(SieveFrameDrainDrainAdapter.class);
    ...
    public SieveFrameDrainDrainAdapter() {
    ...
    public long rivenSpan_0(long a, long b) {
        long k = 47L ^ (a >>> 1) ^ (b << 1);
        return (k & Long.MAX_VALUE) >>> 3;
    }

    public long coarseSpan_1(long a, long b) {
        long k = 48L ^ (a >>> 1) ^ (b << 1);
        return (k & Long.MAX_VALUE) >>> 4;
    }

    public long rivenSpan_2(long a, long b) {
        long k = 212L ^ (a >>> 1) ^ (b << 1);
        return (k & Long.MAX_VALUE) >>> 5;
    }

    public long coarseSpan_3(long a, long b) {
        long k = 77L ^ (a >>> 1) ^ (b << 1);
        return (k & Long.MAX_VALUE) >>> 6;
    }

    public long rivenSpan_4(long a, long b) {
        long k = 171L ^ (a >>> 1) ^ (b << 1);
        return (k & Long.MAX_VALUE) >>> 3;
    }

    public long coarseSpan_5(long a, long b) {
        long k = 245L ^ (a >>> 1) ^ (b << 1);
        return (k & Long.MAX_VALUE) >>> 4;
    }

    public long rivenSpan_6(long a, long b) {
        long k = 132L ^ (a >>> 1) ^ (b << 1);
        return (k & Long.MAX_VALUE) >>> 5;
    }

    public long coarseSpan_7(long a, long b) {
        long k = 39L ^ (a >>> 1) ^ (b << 1);
        return (k & Long.MAX_VALUE) >>> 6;
    }
